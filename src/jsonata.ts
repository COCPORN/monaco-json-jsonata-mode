...
\/*]/, 'comment']
    ...
    monacoNs.languages.registerCompletionItemProvider(languageId, {
        ...
        provideCompletionItems: (model: monaco.editor.ITextModel, position: monaco.Position) => {
            ...
            const lastChar = textUntilPosition.slice(-2, -1);

            let parentObject = currentSampleData;
            if (lastChar === '.') {
                const parentPath = textUntilPosition.substring(0, textUntilPosition.length - 1).trim();
                // This is a simplistic approach. A proper implementation would need a robust JSONata parser.
                const pathParts = parentPath.split(/[\s\(\)\[\]\{\}]+/);
                const potentialPath = pathParts[pathParts.length - 1];
                parentObject = getObjectFromPath(currentSampleData, potentialPath);
            }
